...
class IdenticalMurckoScaffold(BaseDiversityFilter):
    ...
    def update_score(self, dto: UpdateDiversityFilterDTO) -> np.array:
        ...
        for i in score_summary.valid_idxs:
            smile = self._chemistry.convert_to_rdkit_smiles(smiles[i], sanitize=True, isomericSmiles=True)
    ...
    def _calculate_scaffold(self, smile):
        ...
        if mol:
            try:
                ...
                scaffold_smiles = self._chemistry.mol_to_smiles(scaffold, isomericSmiles=True)
            except ValueError:
                scaffold_smiles = ''
        else:
            scaffold_smiles = ''
