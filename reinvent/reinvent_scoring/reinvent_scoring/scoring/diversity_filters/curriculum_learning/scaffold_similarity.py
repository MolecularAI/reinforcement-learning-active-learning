...
from reinvent_scoring.scoring.diversity_filters.curriculum_learning.base_diversity_filter import (
    BaseDiversityFilter,
)
from reinvent_scoring.scoring.diversity_filters.curriculum_learning.memory_record_dto import (
    MemoryRecordDTO,
)
...
class ScaffoldSimilarity(BaseDiversityFilter):
    ...
    def update_score(self, dto: UpdateDiversityFilterDTO) -> np.array:
        ...
        for i in score_summary.valid_idxs:
            ...
            if scores[i] >= self.parameters.minscore:
                loggable_data = (
                    self._compose_loggable_data(dto.loggable_data[i]) if dto.loggable_data else ""
                )
                memory_dto = MemoryRecordDTO(
                    i,
                    dto.step,
                    scores[i],
                    smile,
    ...
    def _find_similar_scaffold(self, scaffold):
        """
        this method tries to find a "similar" scaffold (according to the threshold set by parameter "minsimilarity") and if at least one
