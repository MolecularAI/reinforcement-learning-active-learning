...
class MatchingSubstructure(BaseScoreComponent):
    def __init__(self, parameters: ComponentParameters):
        ...
        self.use_chirality = self.parameters.specific_parameters.get(
            self.component_specific_parameters.USE_CHIRALITY, False)

    def calculate_score(self, molecules: List) -> ComponentSummary:
        score = self._substructure_match(molecules, self.target_smarts, use_chirality=self.use_chirality)
        score_summary = ComponentSummary(total_score=score, parameters=self.parameters)
        return score_summary

    def _substructure_match(self, query_mols, list_of_SMARTS, use_chirality=False):
        if len(list_of_SMARTS) == 0:
            return np.ones(len(query_mols), dtype=np.float32)

        match = [any([mol.HasSubstructMatch(Chem.MolFromSmarts(subst), useChirality=use_chirality)
