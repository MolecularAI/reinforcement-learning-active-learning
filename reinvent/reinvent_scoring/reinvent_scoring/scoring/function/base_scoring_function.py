...
def _update_weights(summary: ComponentSummary, query_length: int, valid_indices: List[int]) -> ComponentSummary:
    if summary.weight is None: 
        summary.weight = np.full(len(valid_indices), 1, dtype=np.float32)
    weight = np.full(query_length, 1, dtype=np.float32)
    assert len(valid_indices) == len(summary.weight) 
    for idx, value in zip(valid_indices, summary.weight):
        weight[idx] = value
...
def _update_total_score(summary: ComponentSummary, query_length: int, valid_indices: List[int]) -> ComponentSummary:
    ...
    print(f'Length of valid indices: {len(valid_indices)}, Length of scores {len(total_score)}')
    ...
    summary = _update_weights(summary, query_length, valid_indices)
    return summary
...
class BaseScoringFunction(ABC):
    ...
    def _score_summary(self, summaries: List[ComponentSummary], smiles: List[str],
        ...
        weight_product = self._compute_weight_product(summaries, smiles) 
        product = penalty * non_penlaty
        final_summary = self._create_final_summary(product, summaries, smiles, valid_indices, weight_product)
        return final_summary

    def _create_final_summary(self, final_score, summaries: List[ComponentSummary], smiles: List[str],
                              valid_indices: List[int], weight_product) -> FinalSummary:

        return FinalSummary(total_score=np.array(final_score, dtype=np.float32),
                            scored_smiles=smiles,
                            valid_idxs=valid_indices,
                            total_weight = np.array(weight_product, dtype=np.float32),
    ...
    def _compute_weight_product(self, summaries: List[ComponentSummary], smiles: List[str]):
        dic = {}
        for summary in summaries:
            if not self._component_is_penalty(summary):
                for index in range(len(summary.weight)):
                    if index not in dic: 
                        dic[index] = summary.weight[index]
                    else: 
                        dic[index] = dic[index] * summary.weight[index]
