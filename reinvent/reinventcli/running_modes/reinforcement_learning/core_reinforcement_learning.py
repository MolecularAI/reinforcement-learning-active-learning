...
from running_modes.reinforcement_learning.logging.base_reinforcement_logger import (
...
class CoreReinforcementRunner(BaseRunningMode):
    ...
    def run(self):
        ...
        for step in range(self.config.n_steps):
            seqs, smiles, agent_likelihood = self._sample_unique_sequences(
                self._agent, self.config.batch_size
            )
            # switch signs
            agent_likelihood = -agent_likelihood
            prior_likelihood = -self._prior.likelihood(seqs)
            score_summary: FinalSummary = (
                self._scoring_function.get_final_score_for_step(smiles, step)
            )
            weight = score_summary.total_weight
            ...
            loss = loss * to_tensor(weight)
            loss, agent_likelihood = self._inception_filter(
                self._agent,
                loss,
                agent_likelihood,
                prior_likelihood,
            ...
            print(loss.sum(), weight.sum(), (loss.sum() / weight.sum()))
    ...
    def _inception_filter(
        self, agent, loss, agent_likelihood, prior_likelihood, sigma, smiles, score
    ):
    ...
    def reset(self, reset_countdown=0):
        ...
        actor_config = ModelConfiguration(
            model_type_enum.DEFAULT, model_regime.TRAINING, self.config.agent
        )
