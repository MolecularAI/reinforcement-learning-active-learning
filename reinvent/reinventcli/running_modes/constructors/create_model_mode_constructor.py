...
from running_modes.configurations import GeneralConfigurationEnvelope, CreateModelConfiguration, \
    LinkInventCreateModelConfiguration, MolformerCreateModelConfiguration

from running_modes.create_model import CreateModelRunner, LinkInventCreateModelRunner, MolformerCreateModelRunner
...
class CreateModelModeConstructor:
    def __new__(cls, configuration: GeneralConfigurationEnvelope) -> BaseRunningMode:
        ...
        elif cls._configuration.model_type == model_type_enum.MOLFORMER:
            config = from_dict(data_class=MolformerCreateModelConfiguration, data=cls._configuration.parameters)
            runner = MolformerCreateModelRunner(configuration=config, logger=logger)
        else:
