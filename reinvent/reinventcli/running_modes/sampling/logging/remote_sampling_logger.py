...
class RemoteSamplingLogger(BaseSamplingLogger):
    ...
    def timestep_report(self, smiles: List[str], canonical_smiles: List[str], **kwargs):
        ...
        fraction_unique_molecules = len(set(canonical_smiles))*100/len(smiles)
        ...
        data = self._assemble_timestep_report(valid_smiles_fraction, fraction_unique_entries,
                                              fraction_unique_molecules, smiles_report, **kwargs)
    ...
    def _assemble_timestep_report(self, fraction_valid_smiles, fraction_unique_smiles,
                                  fraction_unique_molecules, smiles_report,  **kwargs):
        timestep_report = {
            "fraction_unique_smiles": f"{fraction_unique_smiles} %",
            "fraction_valid_smiles": f"{fraction_valid_smiles} %",
            "fraction_unique_molecules": f"{fraction_unique_molecules} %",
            "smiles_report": smiles_report,
            "additional_report": kwargs
        }
