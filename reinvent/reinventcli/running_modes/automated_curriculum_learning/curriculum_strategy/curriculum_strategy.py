...
from reinvent_scoring.scoring.diversity_filters.molformer.diversity_filter import DiversityFilter as MolformerDiversityFilter
...
from running_modes.automated_curriculum_learning.curriculum_strategy.molformer_curriculum_strategy import \
    MolformerCurriculumStrategy
...
class CurriculumStrategy:
    def __new__(cls, prior: GenerativeModelBase, agent: GenerativeModelBase,
        ...
        if curriculum_strategy_enum.NO_CURRICULUM != configuration.name:
            if curriculum_strategy_enum.MOLFORMER != configuration.name:
        ...
        if curriculum_strategy_enum.STANDARD == configuration.name:
            ...
            cl_strategy = LinkInventCurriculumStrategy(prior=prior, agent=agent, configuration=configuration,
                                                       diversity_filter=diversity_filter, logger=logger, inception=inception)
            return cl_strategy
        elif curriculum_strategy_enum.MOLFORMER == configuration.name:
            diversity_filter = MolformerDiversityFilter(configuration.diversity_filter)
            cl_strategy = MolformerCurriculumStrategy(prior=prior, agent=agent, configuration=configuration,
                                                      diversity_filter=diversity_filter, logger=logger, inception=None)
            return cl_strategy
