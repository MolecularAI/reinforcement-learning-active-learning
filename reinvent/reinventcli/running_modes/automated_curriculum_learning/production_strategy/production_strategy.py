...
from running_modes.automated_curriculum_learning.production_strategy.molformer_production_strategy import \
    MolformerProductionStrategy
    ...
    def __new__(cls, prior: GenerativeModelBase, inception: Inception,
        ...
        if production_strategy_enum.STANDARD == configuration.name:
            ...
            production = LinkInventProductionStrategy(prior=prior, diversity_filter=diversity_filter,
                                                      ...
                                                      configuration=configuration, logger=logger)
            return production
        elif production_strategy_enum.MOLFORMER == configuration.name:
            diversity_filter = MolformerDiversityFilter(configuration.diversity_filter)
            production = MolformerProductionStrategy(prior=prior, diversity_filter=diversity_filter,
