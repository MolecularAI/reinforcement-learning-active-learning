...
class Test_BoltzmannWeighting(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._example_mol_paths = (
            PATHS_EXAMPLEDATA.EPSA_BOLTZMANN_WEIGHTING_EXAMPLE_MOLECULE,
            PATHS_EXAMPLEDATA.VECTOR_BOLTZMANN_WEIGHTING_EXAMPLE_MOLECULE,
        )
    ...
    def test_predict_ePSA_with_descriptors(self):
        ...
        conformer = get_mol_as_Conformer(self._example_mol_paths[0])
    ...
    def test_boltzmann_vector(self):
        step_conf = {
            _SBE.STEPID: "01_boltzmann_weighting",
            _SBE.STEP_TYPE: _SBE.STEP_BOLTZMANN_WEIGHTING,
            _SBE.SETTINGS: {
                _SBE.SETTINGS_ARGUMENTS: {
                    _SBE.SETTINGS_ARGUMENTS_PARAMETERS: {
                        _SBWE.PROPERTIES: [
                            {
                                _SBWE.PROPERTIES_INPUT: "conformer_energy",
                                _SBWE.PROPERTIES_OUTPUT: "boltzfactor_ener",
                            }
                        ],
                        _SBWE.WEIGHT: {
                            _SBWE.WEIGHT_INPUT: [
                                "r_mmod_Relative_Potential_Energy-OPLS3e",
                                "Isotropic sheilding",
                                "Anisotropic sheilding",
                                "dD dB contribution"
                            ],
                            _SBWE.WEIGHT_OUTPUT_PREFIX: "bf_weighted",
                            _SBWE.WEIGHT_PROPERTIES: [
        ...
        bweigh_step.get_compounds().append(Compound())
        bweigh_step.get_compounds()[0].add_enumeration(Enumeration(), auto_update=True)
        conformer = get_mol_as_Conformer(self._example_mol_paths[1])
        bweigh_step.data.compounds[0][0].add_conformers(conformer, auto_update=True)
        bweigh_step.execute()
        ...
        self.assertTrue(bweigh_step.get_compounds()[0][0]
            .get_conformers()[0]
            .has_prop("atom.dprop.bf_weighted_Isotropic sheilding_boltzfactor_ener"))

        # check SDF write-out (including Boltzmann-weighted properties as tags)
        out_path = os.path.join(self._test_dir, "turbomol_nmr_reweighted.sdf")
        bweigh_step.write_conformers(out_path)
        stat_inf = os.stat(out_path)
        self.assertGreaterEqual(stat_inf.st_size, 25000)
