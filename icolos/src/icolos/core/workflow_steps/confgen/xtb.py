        ...
        for dir_path, conformer in zip(tmp_dirs, conformers):
            ...
            try:
                for mol in mol_supplier:
                    if mol.HasProp(_COE.TOTAL_ENERGY_TAG):
                        mol.SetProp(_CTE.CONFORMER_ENERGY_TAG, mol.GetProp(_COE.TOTAL_ENERGY_TAG))
                ...
                self._logger.log(
                    f"Failed to parse XTB results for conformer {conformer.get_index_string()}. Original error: {err}",
                    _LE.WARNING,
                )
                raise err
