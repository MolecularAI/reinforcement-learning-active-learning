...
from icolos.utils.general.files_paths import restore_working_dir
    ...
    def execute(self):
        for compound in self.get_compounds():
            for enumeration in compound.get_enumerations():
                if not enumeration.has_molecule():
                    continue

                with TemporaryDirectory() as tmp_dir, restore_working_dir():
                    os.chdir(tmp_dir)
                    # set up
                    ...
                    self._backend_executor.execute(
                        command=_EE.CREST, arguments=settings, check=True
                    )
