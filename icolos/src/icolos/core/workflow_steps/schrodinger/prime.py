...
class StepPrime(StepSchrodingerBase, BaseModel):
    ...
    def _execute_prime(self):
        ...
        while self._subtask_container.done() is False:
            ...
            self._logger.log(f"Using complex_path {complex_paths}, sdf_output {output_sdf_paths}, tmp_output_dir {tmp_dirs}", _LE.DEBUG)
    ...
    def _run_subjob(self, complex_path, sdf_output, tmp_output_dir):
        ...
        self._logger.log(f"Changed to {tmp_output_dir}", _LE.DEBUG)
        arguments = [complex_path, _PE.PRIME_OUTTYPE, _PE.PRIME_OUTTYPE_LIGAND]
        self._logger.log(f"Using default arguments {arguments}", _LE.DEBUG)
        ...
        self._logger.log(f"Final arguments {arguments}", _LE.DEBUG)
        ...
        self._logger.log(f"Program stderr: {result.stderr}", _LE.DEBUG)
        self._logger.log(f"Program stdout: {result.stdout}", _LE.DEBUG)
